...
#		debug($newArray);
	...
	function getTemplateRecord($uid,$renderType,$langUid) {
		if (t3lib_extMgm::isLoaded('templavoila')) {
			...
			if (is_array($rec)) {
				// If print-flag try to find a proper print-record. If the lang-uid is also set,
				// try to find a combined print/lang record, but if not found, the print rec.
				...
				if ($renderType) {
					...
					if (is_array($printRow)) {
						$rec = $printRow;
					} elseif ($rendertype_ref) {
						// Look in rendertype_ref record:
					...
					// If lang_uid is set, try to look up for current language:
					if ($langUid) {
					...
					// If the language uid is set, then try to find a regular record with sys_language_uid
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/templavoila/classes/class.tx_templavoila_htmlmarkup.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/templavoila/classes/class.tx_templavoila_htmlmarkup.php']);
}
?>