 ...
 */
class tx_templavoila_mod2_wizard {
	...
	function wizard_checkDirectory()	{
		...
		if (!@is_dir(PATH_site.$this->pObj->templatesDir))	{
			return
				nl2br('The directory "'.$this->pObj->templatesDir.'" (relative to the website root) does not exist! This is where you must place your HTML templates. Please create that directory <u>before you start the wizard</u>. In order to do so, follow these directions:
	...
	function wizard_step3()	{
		...
		if ($this->wizardData['templateObjectId'])	{
			$url = $this->pObj->cm1Script.'id='.$this->pObj->id.'&table=tx_templavoila_tmplobj&uid='.$this->wizardData['templateObjectId'].'&SET[selectHeaderContent]=0&_reload_from=1&returnUrl='.rawurlencode($this->pObj->mod2Script.'SET[wiz_step]=4');
	...
	function wizard_step4()	{
		$url = $this->pObj->cm1Script.'id='.$this->pObj->id.'&table=tx_templavoila_tmplobj&uid='.$this->wizardData['templateObjectId'].'&SET[selectHeaderContent]=1&_reload_from=1&returnUrl='.rawurlencode($this->pObj->mod2Script.'SET[wiz_step]=5');
		$outputString.= '
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/templavoila/mod2/class.tx_templavoila_mod2_wizard.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/templavoila/mod2/class.tx_templavoila_mod2_wizard.php']);
}
