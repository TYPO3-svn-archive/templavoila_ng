 ...
 * 1096:     function renderTO_editProcessing($singleView,&$row,&$dataStruct,$theFile)
 *
 ...
 * 1382:     function renderTemplateMapper($displayFile,$path,$scope,$dataStruct=array(),$currentMappingInfo=array(),$htmlAfterDSTable='')
 * 1570:     function drawDataStructureMap($scope,&$dataStruct,$mappingMode=0,$currentMappingInfo=array(),$pathLevels=array(),$optDat=array(),$contentSplittedByMapping=array(),$level=0,$tRows=array(),$formPrefix='',$path='',$mapOK=1)
...
#				'borders'  => $GLOBALS['LANG']->getLL('displayModeTable'),
	...
	/**
	 * Removes items from the ->MOD_MENU array. Used for the function menu selector.
	 *
	 * @return	void
	 */
	function menuDisable($DS_row) {
	...
	function main()	{
		...
		switch((string)$this->mode) {
			case 'display': $this->main_display(); break;
			case 'browser': $this->main_browser(); break;
			default:        $this->main_mode   (); break;
		}
	}
	...
	function render_browser() {
		...
		$browserObj =& t3lib_div::getUserObj('EXT:templavoila/cm1/class.tx_templavoila_cm1_browser.php:&tx_templavoila_cm1_browser', '');
		$browserObj->init($this);

		echo $browserObj->renderModuleContent();
	...
	function render_display() {
		...
		echo $displayObj->renderModuleContent();
	...
	function main_mode() {
		...
		$this->content .= $this->doc->header($GLOBALS['LANG']->getLL('mappingTitle'));
		...
		if ($this->returnUrl) {
			$this->content .= '<p><a href="' . htmlspecialchars($this->returnUrl) . '" class="typo3-goBack">' .
				...
				$GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_misc.xml:goBack', 1) .
				'</a></p><hr />';
		}
	...
	function renderDS_info($row) {
		...
		return '
			<dt>' . $GLOBALS['LANG']->getLL('structureRecord') . ': </dt>
	...
	function renderDS_type($row) {
		...
		switch (intval($row['scope'])) {
			case TVDS_SCOPE_PAGE:
				$title = $GLOBALS['LANG']->getLL('structureTypePage');
				$icon = '<img' . t3lib_iconworks::skinImg($this->doc->backPath, 'gfx/i/pages.gif') . ' align="top" alt="" />';
				break;
			case TVDS_SCOPE_FCE:
				$title = $GLOBALS['LANG']->getLL('structureTypeFCE');
				$icon = '<img' . t3lib_iconworks::skinImg($this->doc->backPath, 'gfx/i/tt_content_fce.gif') . ' align="top" alt="" />';
				break;
			default:
				$title = $GLOBALS['LANG']->getLL('structureTypeCustom') . ' [' . $row['scope'] . ']';
				$icon = '<img' . t3lib_iconworks::skinImg($this->doc->backPath, 'gfx/i/unknown.gif') . ' align="top" alt="" />';
				break;
		}

		return '
			<dt>' . $GLOBALS['LANG']->getLL('structureType') . ': </dt>
			<dd>' . $icon . ' ' . $title . '</dd>
		';
	}
	...
	function renderOrigin_details() {
		...
		if ($orgDat['source'] == '_load_ds_xml_to') {
			...
			$tRows[] = '
				<dt>' . $GLOBALS['LANG']->getLL('originXML') . ': </dt>
				<dd>' . t3lib_div::formatSize(strlen($orgDat['subject'])) . 'bytes</dd>
			';
		} else {
			$tRows[] = '
				<dt>' . $GLOBALS['LANG']->getLL('originScratch') . '</dt>
			';
		}
	...
	function renderFile_editProcessing($singleView, $cmd = '') {
		...
		} elseif (t3lib_div::_GP('_save_settings')) {			// Saving settings
			$cmd = 'save_settings';
		...
		if (!count($this->storageFolders))
			$this->msgAppend('error', $GLOBALS['LANG']->getLL('errorNoStorageFolder'));
		...
		if (($cmd == 'load_ds_xml') && ($this->_load_ds_xml_content || $this->_load_ds_xml_to)) {
			...
			if (($toUID = $this->_load_ds_xml_to)) {
				...
				$ds = $this->getDataStructFromDSO($dsREC['dataprot']);
				$sesDat['dataStruct'] = $sesDat['autoDS'] = $ds;
				$sesDat['scope'] = $toREC['scope'];
				...
				$ds = $this->getDataStructFromDSO($this->_load_ds_xml_content);
				$sesDat['dataStruct'] = $sesDat['autoDS'] = $ds;
				$sesDat['scope'] = TVDS_SCOPE_OTHER;
		...
		switch ($cmd) {
			...
			case 'save_settings':
				...
				if (is_array($inputData = t3lib_div::_GP('dataStructureForm', 1))) {
					$dataStruct['meta'] = $this->array_merge_recursive_overrule($dataStruct['meta'], $inputData['meta']);
					$this->cleanUpSettings(&$dataStruct['meta']);

					$sesDat['autoDS']['meta'] =
					$sesDat['dataStruct']['meta'] = $dataStruct['meta'];
					$GLOBALS['BE_USER']->setAndSaveSessionData($this->MCONF['name'] . '_mappingInfo', $sesDat);
				}
				break;
			...
			case 'save_data_mapping':
				/* overwrite only if receiving new assignment */
				if (is_array($inputData = t3lib_div::_GP('dataMappingForm', 1))) {
					$currentMappingInfo = $this->array_merge_recursive_overrule($currentMappingInfo, $inputData);
					$this->cleanUpMappingInfoAccordingToDS($currentMappingInfo, $dataStruct);
			...
			case 'updateDS':
				if (is_array($inDS = t3lib_div::_GP('autoDS', 1))) {
					$dataStruct = $this->array_merge_recursive_overrule($dataStruct, $inDS);
					$this->cleanUpSettings(&$dataStruct['meta']);
				...
				/* interprete bulk-action on _updateDS */
				if (is_array($ucDS = t3lib_div::_GP('_updateDS'))) {
		...
		if (t3lib_div::inList('showXMLDS,save_dsto_into,save_dsto', $cmd)) {
			...
			if ($cmd != 'showXMLDS') {
				...
				if ($this->_save_dsto_spec['type'] == TVDS_SCOPE_PAGE) {
		...
		switch ($cmd) {
			...
			case 'save_dsto_into':
				...
				if (is_array($storeDataStruct['ROOT']['el'])) {
					...
					$presetsObj =& t3lib_div::getUserObj('EXT:templavoila/cm1/class.tx_templavoila_cm1_presets.php:&tx_templavoila_cm1_presets', '');
					$presetsObj->init($this);
					$presetsObj->substEtypeWithRealStuff($storeDataStruct['ROOT']['el'], $contentSplittedByMapping['sub']['ROOT'], $dataArr['tx_templavoila_datastructure']['NEW']['scope']);
				}
				...
				if ($tce->substNEWwithIDs['NEW']) {
					...
					if ($tce->substNEWwithIDs['NEW']) {
						$this->msgAppend('ok', sprintf($GLOBALS['LANG']->getLL('msgDSTOSaved'), $dataArr['tx_templavoila_tmplobj']['NEW']['datastructure'], $this->_load_ds_xml_to = $tce->substNEWwithIDs['NEW'], $this->_save_dsto_spec['pid']));
					} else {
						$this->msgAppend('warning', sprintf($GLOBALS['LANG']->getLL('errorTONotSaved'), $dataArr['tx_templavoila_tmplobj']['NEW']['datastructure']));
					}
				} else {
					$this->msgAppend('warning', $GLOBALS['LANG']->getLL('errorTONotCreated'));
				}

				unset($tce);

				$sesDat['scope'] = $this->_save_dsto_spec['type'];
				$orgDat = array(
					'source' => '_load_ds_xml_to',
					'subject' => $this->_load_ds_xml_to,
					'csum' => md5(serialize($sesDat['autoDS']))
			...
			case 'save_dsto':
				...
				if ($toREC['uid'] && $dsREC['uid']) {
					...
					if (is_array($storeDataStruct['ROOT']['el'])) {
						...
						$presetsObj =& t3lib_div::getUserObj('EXT:templavoila/cm1/class.tx_templavoila_cm1_presets.php:&tx_templavoila_cm1_presets', '');
						$presetsObj->init($this);
						$presetsObj->substEtypeWithRealStuff($storeDataStruct['ROOT']['el'], $contentSplittedByMapping['sub']['ROOT'], $dsREC['scope']);
					}
					...
					$this->msgAppend('ok', sprintf($GLOBALS['LANG']->getLL('msgDSTOUpdated'), $dsREC['uid'], $toREC['uid']));
		...
		$nfos = array(
			TVDS_SCOPE_PAGE  => array($GLOBALS['LANG']->getLL('structureTypePage'  ), $pgei),
			TVDS_SCOPE_FCE   => array($GLOBALS['LANG']->getLL('structureTypeFCE'   ), $fcei),
			TVDS_SCOPE_OTHER => array($GLOBALS['LANG']->getLL('structureTypeCustom'), ''   ),
		);
		...
		switch ($cmd) {
			...
			case 'showXMLDS':
				...
				if (is_array($storeDataStruct['ROOT']['el'])) {
					...
					$presetsObj =& t3lib_div::getUserObj('EXT:templavoila/cm1/class.tx_templavoila_cm1_presets.php:&tx_templavoila_cm1_presets', '');
					$presetsObj->init($this);
					$presetsObj->substEtypeWithRealStuff($storeDataStruct['ROOT']['el'], $contentSplittedByMapping['sub']['ROOT']);
				}
			...
			default:
				if ($BE_USER->check('tables_modify', 'tx_templavoila_datastructure')) {
					...
					if (!$singleView) {
					//	$menuItems[] = '<input type="submit" name="_showXMLDS"     value="' . $GLOBALS['LANG']->getLL('actionShow'   ) . '" title="' . $GLOBALS['LANG']->getLL('actiontitleShow'   ) . '" />';
					...
					//	$menuItems[] = '<input type="submit" name="_preview"       value="' . $GLOBALS['LANG']->getLL('actionPreview') . '" title="' . $GLOBALS['LANG']->getLL('actiontitlePreview') . '" />';
						...
						$menuItems[] = '<input type="submit" name="_save_settings" value="' . $GLOBALS['LANG']->getLL('actionApply'  ) . '" title="' . $GLOBALS['LANG']->getLL('actiontitleApply'  ) . '" />'; if ($this->changedDS) {
						$menuItems[] = '<input type="submit" name="_reload_from"   value="' . $GLOBALS['LANG']->getLL('actionRevert' ) . '" title="' . $GLOBALS['LANG']->getLL('actiontitleRevertS') . '" />'; }
					}
				...
				if ($cmd == 'preview') {
					$content .=
						$this->renderTemplateMapper($this->displayFile, $this->displayPath, $sesDat['scope'], $dataStruct, $currentMappingInfo, $menuContent);
				// show settings in meta-header
				} else if ($cmd == 'meta') {
					$content .=
						$this->renderTemplateSettings($this->displayFile, $this->displayPath, $sesDat['scope'], $dataStruct, $currentMappingInfo, $menuContent);
				} else {
					$content .= '
						<!--
	...
	function renderFile($singleView) {
		...
		if (@is_file($this->displayFile) && t3lib_div::getFileAbsFileName($this->displayFile)) {
			...
			$settingsContent = '
				...
				' .
				$this->doc->section(
					$GLOBALS['LANG']->getLL('structureInfoSettings') . ': ' . $this->cshItem('xMOD_tx_templavoila', 'mapping_meta', $this->doc->backPath, ''),
					$this->renderFile_editProcessing($singleView, 'meta'),
					FALSE,
					TRUE,
					0,
					TRUE);

			$this->parts['settings'] = array(
				'label' => $GLOBALS['LANG']->getLL('tabDSSettings'),
			...
			$xmlContent = '
				...
				' .
				$this->doc->section(
					$GLOBALS['LANG']->getLL('structureInfoXML') . ': ' . $this->cshItem('xMOD_tx_templavoila', 'mapping_ds_showXML', $this->doc->backPath, ''),
			...
			$content =
				$this->doc->section($GLOBALS['LANG']->getLL('templateFile'), 0, 1, 3) .
				$this->msgThrow('error', $GLOBALS['LANG']->getLL('errorFileNotFound'));
		}
	...
	function renderDSO($singleView) {
		...
		if (intval($this->displayUid) > 0) {
			...
			if (is_array($row)) {
				// Turn off header-mapping for FCEs
				$this->menuDisable($row);
				...
				if (is_array($dataStruct)) {
					...
					$content = $this->msgThrow('error', $GLOBALS['LANG']->getLL('noDSDefined'));
				}
				...
				while (false !== ($TO_Row = $TYPO3_DB->sql_fetch_assoc($res)))	{
					...
					if (($fileref = t3lib_div::getFileAbsFileName($TO_Row['fileref']))) {
					...
					$tRows[] = '
							<tr class="bgColor4">
								...
								<td align="right">' . strlen($TO_Row['templatemapping']) . ' bytes</td>
								' . ($BE_USER->check('tables_modify', 'tx_templavoila_datastructure') ? '
								<td align="center"><input type="submit" name="_" value="Edit" onclick="' . htmlspecialchars($onClMsg) . '"/></td>' : '') . '
							</tr>';
				}
				...
				$content .= '
					...
					<div id="c-to">'.
						$this->doc->section(
							$GLOBALS['LANG']->getLL('structureRecordUsed') . ': ' . $this->cshItem('xMOD_tx_templavoila', 'mapping_ds_to', $this->doc->backPath, ''),
				...
				$content = $this->doc->section($GLOBALS['LANG']->getLL('structureInfoParts'), 'ERROR: No Data Structure Record with the UID ' . $this->displayUid, 0, 1, 3);
		}
		else
			$content = $this->doc->section($GLOBALS['LANG']->getLL('structureInfoParts'), 'ERROR: No UID was found pointing to a Data Structure Object record.', 0, 1, 3);
				...
				if ($theFile && @is_file($theFile)) {
					...
					if (is_array($DS_row) || @is_file($DSOfile)) {
						...
						if (is_array($dataStruct)) {
							...
							$scope = !is_array($DS_row) ? intval($DS_row['scope']) : TVDS_SCOPE_OTHER;

							// -----------------------------------------------------
							// -- Processing the head editing --
							list($editContent, $currentHeaderMappingInfo) = $this->renderTO_editProcessing($singleView, $row, $dataStruct, $theFile, 1);
							...
							list($editContent, $currentMappingInfo) = $this->renderTO_editProcessing($singleView, $row, $dataStruct, $theFile, 0);
							...
							$bodyContent = '
								...
								$this->doc->section(
									...
									$this->renderTemplateMapper($theFile, $this->displayPath, $scope, $dataStruct, $currentMappingInfo, $editContent),
									FALSE,
									TRUE,
									0,
									TRUE);
							...
							$content = $this->msgThrow('error', sprintf($GLOBALS['LANG']->getLL('noDSWith'), $dsValue));
					}
					else
						$content = $this->msgThrow('error', sprintf($GLOBALS['LANG']->getLL('noDSWith'), $dsValue));
				}
				else
					$content = $this->msgThrow('error', sprintf($GLOBALS['LANG']->getLL('noDSFile'), $row['fileref']));
			}
			else
				$content = $this->msgThrow('error', sprintf($GLOBALS['LANG']->getLL('noTOWith'), $this->displayUid));
		}
		else
			$content = $this->doc->section($GLOBALS['LANG']->getLL('templateObject'), 0, 1, 3) .
				$this->msgThrow('error', $GLOBALS['LANG']->getLL('errorNoUidFound'));
	 ...
	 * @see renderTO()
	 */
	function renderTO_editProcessing($singleView, &$row, &$dataStruct, $theFile, $headerPart = 0) {
		global $BE_USER;
		...
		if ($cmd == 'save_to')	{
			...
			$this->msgAppend('ok', $GLOBALS['LANG']->getLL('msgMappingSaved'));
		...
		if ($BE_USER->check('tables_modify', 'tx_templavoila_tmplobj')) {
			...
			if ($this->changedTO)
				$this->msgAppend('note', $GLOBALS['LANG']->getLL('msgMappingDiff'));
		...
		return array($menuContent, $headerPart ? $currentMappingInfo_head : $currentMappingInfo);
	}
	...
	function renderHeaderSelection($displayFile, $currentHeaderMappingInfo, $showBodyTag, $htmlAfterDSTable = '') {
		...
		$bodyTagIcon = t3lib_iconWorks::skinImg($this->doc->backPath, t3lib_extMgm::extRelPath('templavoila') . 'res/tags/body.gif', 'width="32" height="17"') . ' alt="" border="0"';
		$bodyTagRow = $showBodyTag ? '
				<tr class="bgColor2">
					<td align="center"><input type="checkbox" name="addBodyTag" value="1"' . ($currentHeaderMappingInfo['addBodyTag'] ? ' checked="checked"' : '') . ' /></td>
					<td align="center"><img' . $bodyTagIcon . ' /></td>
					<td>
						<pre>
							' . htmlspecialchars($html_body) . '
						</pre>
					</td>
				</tr>' : '';
		...
		$headerParts = '
			...
			<table border="0" cellpadding="2" cellspacing="2" id="c-headerParts" style="table-layout: fixed; width: 98%;">
			<colgroup>
	...
	/**
	 * Creates the template settings table + form
	 *
	 * @param	string		The abs file name to read
	 ...
	 * @return	string		HTML table.
	 */
	function renderTemplateSettings($displayFile, $path, $scope, $dataStruct = array(), $currentMappingInfo = array(), $htmlAfterDSTable = '') {
		$meta = array(
			'beLayout'		=> '',
			'beLayoutFile'		=> '',
			'langDisable'		=> '',
		...
		// Data Structure Meta array must be ... and array of course...
		if (is_array($dataStruct) && is_array($dataStruct['meta'])) {
			$meta = $this->array_merge_recursive_overrule($meta, $dataStruct['meta']);
		}
		...
		$content .= '
			...
			<fieldset>
				<legend>FCE settings</legend>

				<dl>
					<dt><label>Language support:' .
						$this->cshItem('xMOD_tx_templavoila', 'settings_languagemode', $this->doc->backPath, '', TRUE) . '
					</label></dt>
					<dd><select onchange="
					switch (this.selectedIndex) {
						case 0: document.getElementById(\'langDisable\').value = \'1\'; document.getElementById(\'langChildren\').value = \'0\'; break;
						case 1: document.getElementById(\'langDisable\').value = \'0\'; document.getElementById(\'langChildren\').value = \'1\'; break;
						case 2: document.getElementById(\'langDisable\').value = \'0\'; document.getElementById(\'langChildren\').value = \'0\'; break;
					}
					">
						<option value="1" ' . ( $meta['langDisable']                           ? 'selected="selected"' : '') . '>Off</option>
						<option value="2" ' . (!$meta['langDisable'] &&  $meta['langChildren'] ? 'selected="selected"' : '') . '>Inherited</option>
						<option value="3" ' . (!$meta['langDisable'] && !$meta['langChildren'] ? 'selected="selected"' : '') . '>Seperated</option>
					</select>
					<input id="langDisable"  type="hidden" name="dataStructureForm[meta][langDisable]"  value="' . ($meta['langDisable' ] ? '1' : '0') . '" />
					<input id="langChildren" type="hidden" name="dataStructureForm[meta][langChildren]" value="' . ($meta['langChildren'] ? '1' : '0') . '" />
					</dd>
				</dl>
			</fieldset>

			<fieldset>
				<legend>TV settings</legend>

				<dl>
					<dt><label>Disable data previews permanently:' .
						$this->cshItem('xMOD_tx_templavoila', 'settings_datapreview', $this->doc->backPath, '', TRUE) . '
					</label></dt>
					<dd><input type="checkbox" name="dataStructureForm[meta][disableDataPreview]" value="1" ' . ($meta['disableDataPreview'] ? 'checked="checked"' : '') . ' /></dd>
					<dt><label>Don\'t show the edit form when a new content element has been created (it will be filled with default values):' .
						$this->cshItem('xMOD_tx_templavoila', 'settings_contentwizard', $this->doc->backPath, '', TRUE) . '
					</label></dt>
					<dd><input type="checkbox" name="dataStructureForm[meta][noEditOnCreation]" value="1" ' . ($meta['noEditOnCreation'] ? 'checked="checked"' : '') . ' /></dd>
					<dt><label>Custom sheet selector:' .
						$this->cshItem('xMOD_tx_templavoila', 'settings_sheetselection', $this->doc->backPath, '', TRUE) . '
					</label></dt>
					<dd><input type="text" name="dataStructureForm[meta][sheetSelector]" value="' . htmlspecialchars(trim($meta['sheetSelector'])) . '" /></dd>
				</dl>
			</fieldset>

			<fieldset>
				<legend>Backend layout</legend>
	 ...
	 * @return	string		HTML table.
	 */
	function renderTemplateMapper($displayFile, $path, $scope, $dataStruct = array(), $currentMappingInfo = array(), $htmlAfterDSTable = '') {
		global $BE_USER;
		...
		$content .= '
			...
			<tbody>
				' . implode('', $this->drawDataStructureMap($scope, $dataStruct['meta'], $dataStruct, 1, $currentMappingInfo, $pathLevels, $optDat, $contentSplittedByMapping)) . '
			</tbody>
			</table>
			' . $htmlAfterDSTable;
		...
		if (($this->mapElPath && !$this->doMappingOfPath) || $this->showPathOnly || $this->_preview) {
			...
			-->
			' .
				$this->getOptsMenuNoHSC() .
				$this->getFuncMenuNoHSC($this->id, 'SET[displayMode]', $this->MOD_SETTINGS['displayMode'], $this->MOD_MENU['displayMode'], '', t3lib_div::implodeArrayForUrl('', $_GET, '', 1, 1)) .
			...
			if ($this->_preview && !$this->mapElPath) {
				...
				$label = $GLOBALS['LANG']->getLL('titleMapping');
	 ...
	 */
	function drawDataStructureMap($scope, $meta, &$dataStruct, $mappingMode = 0, $currentMappingInfo = array(), $pathLevels = array(), $optDat = array(), $contentSplittedByMapping = array(), $level = 0, $tRows = array(), $formPrefix = '', $path = '', $mapOK = 1) {
		global $BE_USER;
		...
		if (is_array($dataStruct)) {
			foreach ($dataStruct as $key => $value) {
				...
				if (is_array($value)) {
					...
					// other than sections and array may be blinded
					if (($value['type'] != 'array') &&
					    ($value['type'] != 'section')) {
					...
					if ($mappingMode) {
						...
						if ($currentMappingInfo[$key]['MAP_EL']) {
							if ($this->doMappingOfPath) {
								$rowCells['htmlPath'] = '&mdash;';
							}
							// If mapping of this information also succeeded...:
							else if (isset($contentSplittedByMapping['cArray'][$key])) {
								...
								$tagIcon = t3lib_iconWorks::skinImg($this->doc->backPath, t3lib_extMgm::extRelPath('templavoila') . 'res/tags/' . $pI['el'] . '.gif', 'height="17"') . ' alt="" border="0"';

								$rowCells['htmlPath'] =
									'<img' . $okayIcon . ' title="' . htmlspecialchars($cF ? sprintf($GLOBALS['LANG']->getLL('mappingFound'), strlen($contentSplittedByMapping['cArray'][$key])) . ($multilineTooltips ? ':' . chr(10) . chr(10) . $cF :'') : $GLOBALS['LANG']->getLL('mappingEmpty' )) . '" class="absmiddle" />' .
						...
						if ($this->mapElPath == ($formPrefix . '[' . $key . ']')) {
							if ($this->doMappingOfPath) {
									...
									if ((($value['type'] == 'attr') && ($pI['modifier'] == 'ATTR')) ||
										...
										if (
											...
											if($k == $currentMappingInfo[$key]['MAP_EL']) {
												$sel = ' selected="selected"'; $didSetSel = 1;
											} else {
												$sel = '';
											}
								...
								$tagIcon = t3lib_iconWorks::skinImg($this->doc->backPath, t3lib_extMgm::extRelPath('templavoila') . 'res/tags/' . $lastLevel['el'] . '.gif', 'height="17"') . ' alt="" border="0"';

								$rowCells['htmlPath'] = '
									<img' . $tagIcon . ' title="---' . htmlspecialchars(t3lib_div::fixed_lgd_cs($lastLevel['path'], -80)) . '" class="absmiddle" />
									<select name="dataMappingForm' . $formPrefix . '[' . $key . '][MAP_EL]" style="width: 5.25em; min-width: 5.25em;">
										' . implode('
										', $opt) . '
								...
								$rowCells['cmdLinks'] = '
									<input type="submit" name="_save_data_mapping" value="' . $GLOBALS['LANG']->getLL('set') . '" />
									<input type="submit" name="_" value="' . $GLOBALS['LANG']->getLL('cancel') . '" />';
							} else {
								$rowCells['cmdLinks'] = '
	 ...
	 * @param	bool		Inheritable element
	 * @param	bool		Localized element
	 * @return	array		Two values, first is addEditRows (string HTML content), second is boolean whether to place row before or after.
	 */
	function drawDataStructureMap_editItem($formPrefix, $key, $value, $level, $path = '', $inheritable = true, $localizable = false) {
		...
		if ($this->editDataStruct && !$this->_preview) {
			if ($this->DS_element == $formPrefix . '[' . $key . ']') {	// If the editing-command points to this element:
				...
				$presetsObj =& t3lib_div::getUserObj('EXT:templavoila/cm1/class.tx_templavoila_cm1_presets.php:&tx_templavoila_cm1_presets', '');
				$presetsObj->init($this);
				$presetsObj->substEtypeWithRealStuff($real);
				...
				<dl id="dsel-general" class="DS-config">
					...
					<dd><select onchange="if (confirm(\'' . $GLOBALS['LANG']->getLL('mess.onChangeAlert') . '\')) document.getElementById(\'_updateDS\').click();"
						...
						<optgroup class="c-divider" label="' . $GLOBALS['LANG']->getLL('structureFormTCEFields') . '">
							...
							<option value="imagelist"'.       ($insertDataArray['tx_templavoila']['eType'] == 'imagelist'        ? ' selected="selected"' : '') . '>Image list</option>
					...
					<dd><select
						...
						<option value=""'              . ($insertDataArray['tx_templavoila']['langOverlayMode'] == ''              ? ' selected="selected"' : '') . '>Default</option>
						<option value="ifFalse"'       . ($insertDataArray['tx_templavoila']['langOverlayMode'] == 'ifFalse'       ? ' selected="selected"' : '') . '>If evaluates to "false"</option>
						<option value="ifBlank"'       . ($insertDataArray['tx_templavoila']['langOverlayMode'] == 'ifBlank'       ? ' selected="selected"' : '') . '>If evaluates to ""</option>
						<option value="never"'         . ($insertDataArray['tx_templavoila']['langOverlayMode'] == 'never'         ? ' selected="selected"' : '') . '>Never</option>
						<option value="removeIfBlank"' . ($insertDataArray['tx_templavoila']['langOverlayMode'] == 'removeIfBlank' ? ' selected="selected"' : '') . '>Remove if evaluates to ""</option>
					</select>
					</dd>
					<dt><label>' . $GLOBALS['LANG']->getLL('structureFormMultilanguage') . ':</label></dt>
					<dd>
						<input type="radio" name="' . $formFieldName . '[tx_templavoila][multilang]" value="1" ' . ( $insertDataArray['tx_templavoila']['multilang'] ? 'checked="checked"' : '') . ' /> yes
						<input type="radio" name="' . $formFieldName . '[tx_templavoila][multilang]" value="0" ' . (!$insertDataArray['tx_templavoila']['multilang'] ? 'checked="checked"' : '') . ' /> no
					</dd>
					' : '') . '
					' : '') . '
				</dl>';
			...
			/*	// The dam-tv-connector will substitute the text above, that's $%*%&"$%, but well anyway, let's not break it
				...
				if (($insertDataArray['type'] != 'array') &&
					...
					<dl id="dsel-tce" class="DS-config">
						...
						<dt><label>TCE Multilanguage:</label></dt>
						<dd>	<input type="radio" name="' . $formFieldName . '[TCEforms][multi]" value="1" ' . ( $insertDataArray['TCEforms']['multi'] ? 'checked="checked"' : '') . ' /> yes
							<input type="radio" name="' . $formFieldName . '[TCEforms][multi]" value="0" ' . (!$insertDataArray['TCEforms']['multi'] ? 'checked="checked"' : '') . ' /> no</dd>
...
<!--					<input type="submit" name="' . $formFieldName . '" value="' . $GLOBALS['LANG']->getLL('delete') . '" />  -->
			...
			switch ($type) {
				case 'TypoScriptObject':
					...
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
						'*',
						'pages',
						'storage_pid IN (' . $this->storageFolders_pidList . ')' . t3lib_BEfunc::deleteClause('pages'),
						'',
						'title'
					);
					...
					$output .=
						...
						<div style="width: 94.7%;" class="tv-ts-tree">
							...
							<iframe width="100%" height="400" style="border: 0;" id="browser[communication]" src="' . $this->baseScript . 'mode=browser&pid=' . $pid . '&current=' . $curValue['objPath'] . '"></iframe>
						</div>
					';
	...
	function cleanUpMappingInfoAccordingToDS(&$currentMappingInfo, $dataStruct) {
		if (is_array($currentMappingInfo)) {
			foreach($currentMappingInfo as $key => $value) {
				...
				} else if (is_array($dataStruct[$key]['el'])) {
	...
	/**
	 * Function to clean up "old" stuff in the meta array. Basically it will remove EVERYTHING which is not known according to the input Data Structure
	 *
	 * @param	array		Data Structure meta
	 * @return	void
	 */
	function cleanUpSettings(&$currentMeta) {
		if (is_array($currentMeta)) {
			foreach($currentMeta as $key => $value) {
				if (trim($currentMeta[$key]) === '') {
					unset($currentMeta[$key]);
				} else if (is_array($currentMeta[$key])) {
					$this->cleanUpSettings($currentMeta[$key]);
				}
			}
		}
	}
...
class tx_templavoila_cm1_integral extends tx_templavoila_cm1 {
	...
	function menuConfig() {
		...
		if (t3lib_div::_GP('mode') == 'display' ||
			...
			$this->MOD_MENU['page'] =
				array(
					'settings'  => $GLOBALS['LANG']->getLL('tabFileSettings'),
	 ...
	 */
	function menuDisable($DS_row) {
		parent::menuDisable($DS_row);

		if (intval($DS_row['scope']) == TVDS_SCOPE_FCE) {
			unset($this->MOD_MENU['page']['header']);
			if ($this->MOD_SETTINGS['page'] == 'header')
				$this->MOD_SETTINGS['page'] = 'mapping';
		}
	}
	...
	function getFuncMenuNoHSC($mainParams, $elementName, $currentValue, $menuItems, $script = '', $addparams = '') {
		if (is_array($menuItems)) {
			...
			if (count($options)) {
				$onChange = 'jumpToUrl(\'' . $script . '&mapElPath=' . $this->mapElPath . '&' . $elementName . '=\' + this.options[this.selectedIndex].value, this);';
				return '
	...
	function getOptsMenuNoHSC() {
		...
		/* general option-group */
		{
			...
			$entries[] = '<li class="mradio' . ( $this->MOD_SETTINGS['displayTags'] == 2 ? ' selected' : '') . '" name="displayTags"><a href="' . str_replace('###', '2', $link) . '"' . '>' . 'Inline elements'  . '</a></li>';
			$entries[] = '<li class="mradio' . ( $this->MOD_SETTINGS['displayTags'] == 3 ? ' selected' : '') . '" name="displayTags"><a href="' . str_replace('###', '3', $link) . '"' . '>' . 'Form elements'    . '</a></li>';
			$entries[] = '<li class="mradio' . ( $this->MOD_SETTINGS['displayTags'] == 4 ? ' selected' : '') . '" name="displayTags"><a href="' . str_replace('###', '4', $link) . '"' . '>' . 'Graphic elements' . '</a></li>';
			$entries[] = '<li class="mradio' . ( $this->MOD_SETTINGS['displayTags'] == 5 ? ' selected' : '') . '" name="displayTags"><a href="' . str_replace('###', '5', $link) . '"' . '>' . 'Table elements'   . '</a></li>';
			$entries[] = '<li class="mradio' . ( $this->MOD_SETTINGS['displayTags'] == 6 ? ' selected' : '') . '" name="displayTags"><a href="' . str_replace('###', '6', $link) . '"' . '>' . 'List elements'    . '</a></li>';
	...
	function main()	{
		...
		//	$this->content .= $this->doc->spacer(10);
			...
			// Selecting display or module mode:
			switch((string)$this->mode) {
				case 'display': $this->render_display();  break;
				case 'browser': $this->render_browser();  break;
				default:        $this->render_mode(true); break;
			}
